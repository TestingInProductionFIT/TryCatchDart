...
import 'dashboard_screen.dart' show showWidgetPicker;
...
/// respect each widget's minimum size. In edit mode widgets can be dragged
...
class _WorkspaceGridState extends ConsumerState<WorkspaceGrid> {
  ...
  Widget _buildLeaf({
    ...
    final typeId = _typeOf(workspace, widgetId);
    final descriptor = WidgetRegistry.byId(typeId);
    if (descriptor == null) return const SizedBox.shrink();

    final isDragged = _dragLeafId == widgetId;
    ...
    Widget card = AppCard(
            ...
            ? Row(
                ...
                children: [
                  ...
                  _HeaderIconButton(
                    ...
                    tooltip: 'Split this tile and add a widget',
                    onTap: () => showWidgetPicker(
                      context,
                      ref,
                      splitWidgetId: widgetId,
                    ),
                  ),
                  _HeaderIconButton(
                    icon: Icons.close,
                    ...
                    onTap: () {
                      ref.read(workspaceProvider.notifier).removeWidget(widgetId);
                    },
                  ),
                ],
              )
      ...
      child: widget.editMode
          // Block widget interactivity while layout handles are active.
    ...
    if (widget.editMode) {
      card = GestureDetector(
        ...
        onPanUpdate: (details) {
          ...
          final target = _leafAt(_dragPointer, widgetId);
        ...
        onPanEnd: (_) {
          ...
          if (target != null) {
            ref
                ...
                .swapWidgets(widgetId: widgetId, ontoWidgetId: target);
          }
          setState(() {
            _dragLeafId = null;
            _swapTargetId = null;
...
/// Small badge that follows the pointer while dragging a widget in edit mode.
...
/// Small icon button for the edit-mode card header (split, remove).
