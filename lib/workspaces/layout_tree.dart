...
/// widgets. Split directions alternate with depth so new splits rotate
/// orientation, and every widget type declares a minimum pixel size that the
...
/// Per-widget-type minimum size in logical pixels (from `WidgetRegistry`).
typedef MinSizeLookup = Size Function(String typeId);
...
sealed class LayoutNode {
  ...
  /// All widget ids in this subtree, in layout order.
...
/// Leaf: one widget instance.
class LeafNode extends LayoutNode {
  final String widgetId;
  final String typeId;

  LeafNode({required this.widgetId, required this.typeId});

  @override
  List<LeafNode> get leaves => [this];

  @override
  Size minSize(MinSizeLookup minOf) => minOf(typeId);

  @override
  Map<String, dynamic> toJson() =>
      {'type': 'leaf', 'widgetId': widgetId, 'typeId': typeId};
}
...
void _layoutNode(
  ...
  if (node is LeafNode) {
    leafRects[node.widgetId] = bounds;
    return;
  }
...
/// Inserts a new leaf for [typeId] by splitting the largest-area leaf.
///
...
({LayoutNode root, String widgetId})? insertLeaf(
  LayoutNode? root,
  String typeId,
  String newWidgetId,
  MinSizeLookup minOf,
  ...
  if (root == null) {
    return (root: LeafNode(widgetId: newWidgetId, typeId: typeId), widgetId: newWidgetId);
  }
  ...
  final newLeaf = LeafNode(widgetId: newWidgetId, typeId: typeId);
  final result = _splitLeaf(root, target.widgetId, newLeaf, minOf);
  return (root: result, widgetId: newWidgetId);
}

/// Splits the leaf [widgetId] into two, placing [newWidgetId] with [typeId]
/// beside it. Returns `null` when [widgetId] is not in the tree.
({LayoutNode root, String widgetId})? splitLeaf(
  LayoutNode? root,
  String widgetId,
  String typeId,
  String newWidgetId,
  ...
  if (!root.leaves.any((l) => l.widgetId == widgetId)) return null;
  final newLeaf = LeafNode(widgetId: newWidgetId, typeId: typeId);
  return (
    root: _splitLeaf(root, widgetId, newLeaf, minOf),
...
/// Replaces the leaf with [widgetId] by a split containing it and [newLeaf].
LayoutNode _splitLeaf(
  ...
  if (node is LeafNode) {
    ...
    final rect = _lastKnownRects[widgetId];
  ...
  if (node is SplitNode) {
    return node.withChildren(
      a: _splitLeaf(node.a, widgetId, newLeaf, minOf),
      b: _splitLeaf(node.b, widgetId, newLeaf, minOf),
    );
  }
  return node;
}
...
/// Removes the leaf with [widgetId]; a parent split with a single remaining
/// child collapses into that child.
LayoutNode? removeLeaf(LayoutNode? root, String widgetId) {
  if (root == null) return null;
  if (root is LeafNode) {
    return root.widgetId == widgetId ? null : root;
  }
  if (root is! SplitNode) return null;

  final newA = removeLeaf(root.a, widgetId);
  final newB = removeLeaf(root.b, widgetId);
...
/// Swaps the *content* of two leaves (drag a widget onto another).
LayoutNode? swapLeaves(LayoutNode? root, String widgetId, String otherId) {
  if (root == null || widgetId == otherId) return root;

  LeafNode? first;
  LeafNode? second;
  void collect(LayoutNode node) {
    if (node is LeafNode) {
      if (node.widgetId == widgetId) first = node;
      if (node.widgetId == otherId) second = node;
  ...
  LayoutNode replace(LayoutNode node) {
    if (node is LeafNode) {
      if (node.widgetId == widgetId) {
        return LeafNode(widgetId: source.widgetId, typeId: source.typeId);
      }
      if (node.widgetId == otherId) {
        return LeafNode(widgetId: target.widgetId, typeId: target.typeId);
      }
      return node;
    }
    if (node is SplitNode) {
...
/// Builds a balanced alternating tree from an ordered widget list — used to
/// migrate persisted grid layouts (and as a factory default).
