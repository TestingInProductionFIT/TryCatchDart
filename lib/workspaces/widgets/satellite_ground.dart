...
Future<Uint8List?> _cachedFetcher(Uri url) async {
  ...
  if (bytes != null && cache.isSupported) {
    try {
      await cache.putTile(
        url: key,
...
Future<(Uint8List? bytes, int qx, int qy, int qz)> _fetchTileImage(
  ...
  Uri url(int x, int y, int z) => Uri.parse(
...
Future<SatellitePatch?> _fetch(
  ...
  final recorder = ui.PictureRecorder();
  final canvas = ui.Canvas(recorder);
  // Dark underlay so missing tiles read as unloaded imagery, not white.
  canvas.drawRect(
    ui.Rect.fromLTWH(0, 0, cols * 256, rows * 256),
    ui.Paint()..color = const ui.Color(0xFF20242A),
  );
