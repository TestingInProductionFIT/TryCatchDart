...
/// widget so they all behave identically.
class TimeSeriesConfig {
  ...
  /// Whether to render the series legend (off when the host widget shows its
      ...
      for (var i = 0; i < widget.config.series.length; i++) ...[
        LineChartBarData(
          ...
          belowBarData: widget.config.series.length == 1
              ? BarAreaData(
                  show: true,
                  color: widget.config.series[i].color.withValues(alpha: 0.08),
                )
        ...
        Expanded(
          child: Stack(
            ...
            children: [
              ...
              if (samples.isEmpty)
                const Center(
                  child: WaitingForData(compact: true),
                ),
            ],
          ),
        ),
