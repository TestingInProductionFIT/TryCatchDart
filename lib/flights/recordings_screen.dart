...
class _RecordingsScreenState extends ConsumerState<RecordingsScreen> {
  ...
  /// v1+ headers yield duration/packets/max-alt straight from 112 bytes —
  /// only legacy files wait for their card decode to fill those in.
...
class _RecordingCardState extends State<_RecordingCard> {
  ...
  Widget build(BuildContext context) {
    ...
    final stats = <String>[
      ...
      if (info.maxAltM != null)
        'max ${info.maxAltM!.toStringAsFixed(info.maxAltM!.abs() >= 100 ? 0 : 1)} m',
      _sizeLabel,
    ];
    final date =
        ...
        children: [
          Row(
            children: [
              Icon(
                ...
                size: 18,
                color:
                    isLoaded ? AppColors.primary : AppColors.mutedForeground,
              ),
              const SizedBox(width: 8),
              Expanded(
                child: Column(
          ...
          Row(
            children: [
              ...
              OutlinedButton.icon(
                onPressed:
                    info.durationMs != null && info.durationMs! > 2000
                        ? widget.onTrim
                        : null,
