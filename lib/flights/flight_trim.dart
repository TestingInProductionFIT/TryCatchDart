...
///
/// Recording files open with a fixed [RecordingHeader] (v1+; legacy files
/// are headerless) followed by a flat stream of chunks: a 12-byte header
...
///
/// The destination gets a fresh v1 header: stats recomputed over the kept
/// slice, launch site propagated from the source header when present.
Future<int> trimRecording({
  ...
  await writeRecordingChunks(dstPath, kept);
  final srcHeader = await tryReadRecordingHeader(srcPath);
  await finalizeRecordingFile(dstPath, launch: srcHeader?.launchRef);
  return kept.length;
}
...
///
/// The framing comes from the v1 file header; headerless files yield an
...
Future<DecodedFlight> decodeRecordingFrames(String path) async {
  ...
  if (framing != 52 && framing != TelemetryFraming.payloadLength) {
